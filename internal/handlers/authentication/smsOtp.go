...
func (ah authHandler) SendSmsOTP() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		date, errSql := ah.svc.AuthRepositoryInstance().GetDate(ctx)
		if errSql.ErrorCode != 0 {
			go utils.Logger(ctx, ah.Logger.Error, requestDesc, serviceDesc, requestID, errSql)
			response.Json(w, http.StatusSeeOther, err.Error())
			return
		}

		otpTxt := fmt.Sprintf(ah.svc.ConfigInstance().GetString("api.otp.otpText"), usr.Security.TwoFA.SMSotp.Code, date)
		
		_, err = ah.svc.TwoFaInstance().SmsOTP(usr.Phone, otpTxt)
		if err != nil {
